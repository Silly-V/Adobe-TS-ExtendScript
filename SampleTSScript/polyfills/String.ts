interface String {
	/**
	 * Takes leading and trailing whitespaces off a string.
	 */
	trim (): string;
	
	/**
	 * Splices in a string at a specified index, could be used for single-character replacement or splicing in a multi-character string.
	 * @param index - Which character to replace.
	 * @param replacement - A string to replace the character with.
	 */
	...
	/**
	 * Encodes a hexadecimal-encoded string.
	 */
	hexEncode (): string;
	...
	/**
	 * Ensures any RegEx characters in a string will be escaped to become raw characters.
	 * Used when the string will be fed into a `new RegEx()` constructor.
	 */
	 ...
	 */
	includes (inStr: string, start?: number): boolean;

	startsWith (str: string, rawPos?: number): boolean;
	endsWith (str: string, rawPos?: number): boolean;
}
